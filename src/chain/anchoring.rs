...
impl StateAnchorer {
        ...
        loop {
            tokio::select! {
                ...
                _ = shutdown_rx.recv() => {
                    ...
                    // v0.45.0 (spec 13 §6.3): opt-in graceful pause.
                    // Only fires when explicitly enabled — the wallet
                    // key is in memory by virtue of running the anchor
                    // loop, so this doesn't widen the at-rest surface;
                    // it just adds a single signed call on the way out.
                    if self.config.pause_on_shutdown {
                        match self.submit_pause_for_shutdown().await {
                            Ok(tx_hash) => info!(
                                tx_hash = %tx_hash,
                                "pauseNode broadcast on shutdown — node will appear paused to peers and dashboard consumers"
                            ),
                            Err(e) => warn!(
                                error = %e,
                                "pauseNode on shutdown failed — node will still exit (operator should manually pause from the dashboard if needed)"
                            ),
                        }
                    }
                    break;
    ...
    async fn perform_anchor(&mut self) -> Result<String> {
        ...
        // Step 2: Build SC call data (6 args — timestamp is generated by the SC)
        ...
        // Steps 3–6: nonce + raw TX build + send + decode + sign + broadcast.
        // Shared with the SIGTERM `pauseNode` path (`submit_pause_for_shutdown`).
        let tx_hash_hex = self.submit_signed_sc_invoke(&call_data).await?;
    ...
    /// Build + send + decode + sign + broadcast an SC-invoke TX whose
    /// SC call data is `call_data` (e.g. `"anchorState@..."` or
    /// `"pauseNode"`). Returns the broadcast tx-hash on success.
    ///
    /// Shared between `perform_anchor` and `submit_pause_for_shutdown`
    /// (the SIGTERM `pauseNode` path). The post-call-data flow is
    /// identical: fetch nonce → build raw TX → /transaction/send →
    /// /transaction/decode → Ed25519-sign the tx-hash → /transaction/broadcast.
    async fn submit_signed_sc_invoke(&self, call_data: &str) -> Result<String> {
        let nonce = self.get_nonce().await?;
        let data_b64 = BASE64.encode(call_data.as_bytes());
        ...
        // Klever /transaction/send can return HTTP 200 with both error
        // and data fields. Only treat HTTP-level errors (5xx, network)
        // as failures here; we surface a body-embedded error below
        // when the result field is missing.
        ...
        let mut broadcast_tx = tx_result;
    ...
    /// Sign and broadcast a `pauseNode` SC TX from the anchorer wallet.
    ///
    /// Invoked by the SIGTERM handler when `[anchoring] pause_on_shutdown = true`
    /// (spec 13 §6.3). Operator-visible effect: the node signals to the
    /// network — and to dashboard consumers via `isNodePaused` — that
    /// it is going offline gracefully rather than crash-stopping. On
    /// next startup the operator can resume manually from the
    /// Anchoring tab.
    ///
    /// Best-effort: shutdown proceeds even if this fails. The caller
    /// (the `run()` shutdown branch) bounds the call by the outer
    /// shutdown-timeout in `Node::run`.
    ///
    /// **Wallet-safety scope (spec 12 §5.2 v0.45.0 row):** this is the
    /// first node-side SC-signing path. The `wallet_key` lives in
    /// process memory for the lifetime of `StateAnchorer`, same as
    /// the anchor loop. Operators who would rather scope the key to
    /// the anchor loop only must keep `pause_on_shutdown = false`
    /// (the default).
    // pub(super) so only `run()`'s shutdown branch can call this —
    // external callers risk double-broadcast (nonce conflict, wasted
    // KLV) per Code Audit W5.
    pub(super) async fn submit_pause_for_shutdown(&self) -> Result<String> {
        if self.klever.node_url.is_empty() || self.klever.contract_address.is_empty() {
            anyhow::bail!(
                "cannot send pauseNode: klever.node_url or klever.contract_address not configured"
            );
        }
        // Spec 12 §2.11: `pauseNode(reason: ManagedBuffer)`. SIGTERM-
        // initiated pauses always carry the same reason so operators
        // and dashboard consumers can distinguish a graceful auto-
        // pause from a manual operator pause. The SC's 256-byte cap
        // is far above our fixed string. Reason is hex-encoded as
        // the single argument: pauseNode@<hex>.
        const SHUTDOWN_REASON: &str = "graceful-shutdown";
        let call_data = format!("pauseNode@{}", hex::encode(SHUTDOWN_REASON.as_bytes()));
        self.submit_signed_sc_invoke(&call_data).await
    }
