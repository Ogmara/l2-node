...
- **Federate now REFRESHES a federated channel's metadata** (not just create-if-absent),
  so a federated private channel picks up a logo/display-name set after the first
  federation. A channel this node HOSTS (record without `federated_from`) is still never
  touched. Pairs with the client re-federating on an invite-link join.
...
- **Signature-verification rejection now logs `author`, `msg_type`, `ts`,
  `payload_len`, `sig_len`** instead of a bare "signature verification failed for
  both formats". Diagnostics-only — lets operators see exactly which message (and
  from which wallet/type) a peer rejected on the gossip path, instead of guessing.

## [0.68.0] - 2026-06-12

Loopback-only E2E diagnostics endpoint (for "can't decrypt" / vanishing-DM
investigation).

### Added

- **`GET /admin/debug/e2e?wallet=<addr>&peer=<addr>`** (loopback-only, same guard
  as `mesh-stats`). Read-only dump: the wallet's device enc-key directory
  (`enc_keys` — two entries sharing a `device_id` is the wrap-collision
  signature), and for the conversation (scope from `wallet`+`peer`, or explicit
  `scope=<64-hex>`): the stored wrapped-key envelopes (`channel_keys`, decoded
  `target/author/device_id/epoch`) and the conversation's stored DM messages
  (`dm_messages`, with `has_envelope` — `false` ⇒ indexed but body missing, i.e.
  vanishes from history on reload). Public key material + ciphertext metadata
  only; no private keys or plaintext.
